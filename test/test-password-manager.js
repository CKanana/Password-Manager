...
function expectReject(promise) {
  return promise.then(
    (result) => {
      throw new Error(`Expected failure, but function returned ${result}`);
    },
    () => {}
  );
}
...
describe("Password manager", function () {
  ...
  describe("functionality", function () {
    ...
    it("can dump and restore the database", async function () {
      ...
      const [contents, checksum] = data;
      const newKeychain = await Keychain.load(password, contents, checksum);
